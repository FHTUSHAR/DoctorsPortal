...
const AvailableAppointment = ({ selectedDate, setSelectedDate }) => {
    ...
    const { data: appointmentOptions = [], refetch, isLoading } = useQuery({
        ...
        queryFn: () => fetch(`https://doctors-portal-server-lime-nu.vercel.app/appointmentOptions?date=${date}`)
            .then(res => res.json())

    })

    if (isLoading) {
        return <div role="status" className=' flex justify-center text-center'>
